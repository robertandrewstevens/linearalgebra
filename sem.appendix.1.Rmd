...
To find the variance/covariance matrix, we can first find the the inner product of the means centered matrix with itself and divide by (n - 1). We can compare this result to the result of the cov function (the normal way to find covariances).
...
we can solve for b by multiplying both sides of the equation by X^−1^ or YX^−1^ = bXX^−1^ = b
...
In the next example we consider 5 extraversion items for 200 subjects collected as part of the Synthetic Aperture Personality Assessment project. The items are taken from the *International Personality Item Pool* (http://ipip.ori.org). The data are stored at the http://personality-project.org web site and may be retrieved in R. Because the first column of the data matrix is the subject identification number, we remove this before doing our calculations.
...
If we want to find the intercept as well, we can add a column of ones to the X matrix.
